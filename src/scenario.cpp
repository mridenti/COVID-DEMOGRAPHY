...
void driver2D_simple(DerivFunc derivs, double y0[][NA], ScenarioParameters *p, ScenarioOutput *o)
{
    ...
    double h = 1.0, t=0.0, y[NEA][NA] = { { 0 } }, dydt[NEA][NA];
    double yout[NEA][NA];
    ...
    for (int i = 1; i < MAX_DAYS; i++)
    {
        p->day = i;
        derivs(t, y, dydt, p);
        rk42D(y, dydt, t, h, yout, derivs, p);
...
int write_output(const char* filename, ScenarioOutput *o)
{
	...
	// Print header
	fprintf(result, "S,E,I,R,N,A,C,H,L,Qi,Qa,");
	char* paramNames[] = { "S", "E", "I", "R", "N", "A", "C", "H", "L", "Qi", "Qa" };
	for (int k = 0; k < NEA; k++)
	{
    ...
    fprintf(result, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,",
              Y_sum[Var::S][day], Y_sum[Var::E][day], Y_sum[Var::I][day], Y_sum[Var::R][day], Y_sum[Var::N][day]
            , Y_sum[Var::A][day], Y_sum[Var::C][day], Y_sum[Var::H][day], Y_sum[Var::L][day], Y_sum[Var::Qi][day], Y_sum[Var::Qa][day]);
    for (int k = 0; k < NEA; k++)
    {
        fprintf(result, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,", YOUT[k][Var::S][day], 
                YOUT[k][Var::E][day], YOUT[k][Var::I][day],
                YOUT[k][Var::R][day], YOUT[k][Var::N][day], 
                YOUT[k][Var::A][day], YOUT[k][Var::C][day], 
                YOUT[k][Var::H][day], YOUT[k][Var::L][day], YOUT[k][Var::Qi][day], YOUT[k][Var::Qa][day]);
    }
